...
from krishna.utils.database import get_assistant, clonebotdb
from krishna.utils.database.clonedb import has_user_cloned_any_bot
from config import LOGGER_ID, CLONE_LOGGER
import requests
from krishna.utils.decorators.language import language
import pyrogram.errors

from krishna.utils.database.clonedb import get_owner_id_from_db
    ...
    if len(message.command) > 1:
        ...
        try:
            ai = Client(
                ...
                API_HASH,
                bot_token=bot_token,
                plugins=dict(root="krishna.cplugin"), 
            )
            await ai.start()
            bot = await ai.get_me()
